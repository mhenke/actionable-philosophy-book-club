  ...
  function initGlobalBar() {
    ...
    function makeIconBtn({ id, svg, label, ariaLabel, labelFont, onClick }) {
      ...
      b.innerHTML = svg + (label
        ? `<span class="icon-btn-label" style="display:inline-block;max-width:0;opacity:0;margin-left:0;overflow:hidden;font-family:${labelFont || FONT};transition: width 0.25s ${EASE}, opacity 0.2s ease, margin-left 0.25s ${EASE};">${label}</span>`
        : '');
