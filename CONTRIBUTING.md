...
### The Drafts staging folder (`meetings/drafts/`)

`meetings/drafts` is a staging area for materials that are being prepared before a meeting has a confirmed number. It does not appear in the `MEETINGS` manifest and its content is not shown on the dashboard. See docs/adr/0009-plain-language-section-naming.md for background on the terminology change.
