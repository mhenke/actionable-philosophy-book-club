...
def parse_args():
    ...
    parser.add_argument("--type", choices=['deep-dive', 'critique', 'debate'], help="Type of recording (recordings only).")
...
def main():
    ...
    # Detect alternate patterns (-alt, _alt, -alternate, _alternate) case-insensitively
    is_alternate = False
    ...
    if category == 'recording' and args.type:
        filename_parts.append(args.type)
    if is_alternate:
